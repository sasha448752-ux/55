...
const createConversationToken = () => {
  if (window.crypto?.randomUUID) return window.crypto.randomUUID();
  if (!window.crypto?.getRandomValues) throw new Error('Чат требует защищённое подключение. Откройте сайт по HTTPS.');
...
const conversationToken = () => {
  ...
  if (!token) { token = createConversationToken(); localStorage.setItem(chatStorageKey, token); }
  return token;
};
